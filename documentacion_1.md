# OpenSoul — Guía Técnica Completa (V2 Jurídica & 3D)

> **Proyecto:** OpenSoul (LSB Legal App)
...
> **Contexto Principal:** Herramienta enfocada en el entorno jurídico y administrativo (denuncias, comisarías, juzgados).
...
Las personas sordas en Bolivia enfrentan enormes barreras de comunicación crítica en contextos legales y administrativos. OpenSoul actúa como un puente bidireccional, pero ahora fuertemente inclinado hacia la exactitud y seguridad jurídica:

- **LSB → Texto/Audio/Avatar 3D:** Una persona sorda selecciona tarjetas judiciales (como "DENUNCIAR", "ROBO", "AYER"). La app envía estas glosas a AWS. Para evitar alucinaciones, un **Motor Híbrido Propio** procesa directamente las reglas y crea una frase inmutable. Luego, una IA generativa (Bedrock) pule la gramática. Finalmente, Polly genera un audio MP3 y el Frontend dispara un **Avatar 3D secuencial** reproduciendo la frase en señas visualmente.
...
│                                                                  │
│  Usuario selecciona secuencia LSB ("YO", "DENUNCIAR")           │
│  → Presiona "TRADUCIR Y GENERAR"                                │
│  → DataSource hace POST HTTP a AWS API Gateway                  │
│  → Lambda extrae Roles Temáticos (Sujeto, Verbo, Objeto)        │
│  → Lambda construye 'baseSentence' estricta                     │
│  → Bedrock pule la gramática a 'generatedText'                  │
...
│   ├── lambda_mock.py                # Lambda simplificada antigua
...
│   │   └── theme.dart                # Tema "Dark Institucional Jurídico"
│   │
│   └── features/
...
│       │   │
│       │   ├── data/                 
│       │   │   ├── datasources/local_cards_datasource.dart # Base de datos local: 62 Tarjetas
...
│       │       ├── screens/home_screen.dart                # Pantalla principal (Layout rediseñado sin overflow)
│       │       └── widgets/
│       │           ├── card_grid.dart                      # Grilla reponsiva 3 columnas con shrinkWrap
│       │           ├── category_filter.dart                # Chips de 9 categorías jurídicas
│       │           ├── sentence_builder.dart               # Barra de secuencias numeradas
│       │           └── avatar_sign_viewer.dart             # El motor visual 3D usando GLB
│       │
│       └── audio_to_lsb/            # Módulo de Audio a Señas (Para referencia estructural)
│
...
#### `theme.dart`
- **Propósito:** Configuración central visual de tipo "Dark Institucional". Colores basados en tonalidades azules de fondo (`#0D1117`) y dorados (`#FFD700`) como acentos para dar sensación de oficialidad a una app de uso judicial.
...
- **Qué hace:** Actúa como la memoria RAM de tarjetas en 9 Categorías Penales (Agresores, Delitos, Urgencias, Servicios). Pasó de tener 13 tarjetas de prueba a contener **62 tarjetas especializadas**.

##### `remote_translation_datasource.dart`
- **Qué hace:** Solo un objetivo: Hacer `HTTP POST`. Envía `{'context': 'legal', 'cards': [...]}` hacia el API Gateway. Extrae la compleja respuesta anidada del JSON. Este archivo es el límite del aislamiento de la red.
...
- **Su lógica:** Mantiene un diccionario local `_animationMap` en donde, por ejemplo, asocia la palabra `'DENUNCIAR'` con el bloque de animación `'Pointing'`. El widget avanza a la siguiente animación cada poco tiempo guiado por una barra de progreso que lee del Array `glossSequence`.
...
#### A. Diccionario Temático y Motor Propio
- `GLOSS_LEXICON` mapea "ROBO" categorizándolo como "objeto_delito".
- `generate_base_sentence()` coge estos tokens. Nunca "adivina", rellena plantillas duras de Python. Esto asegura que nadie denuncie falsamente un crimen al sistema judicial a causa de una alucinación de GPT/Claude.

#### B. Anthropic Claude (Bedrock) Fallback
Si el texto base es muy rígido, se solicita a `aws bedrock-runtime` mejorar la redacción. Si falla, el bloque Try-Catch en Lambda garantiza un retorno de 200 usando la frase base.
...
```
Ese MP3 se encripta y tiene muerte programada en 1 hora, salvando los dolores de cabeza de permisos `403 Forbidden` / Format Error 4 originados desde Dart en la web.
...
```text
1. Usuario elige: ["YO", "ROBO", "AYUDA"]
2. Backend devuelve: glossSequence = [{"gloss": "YO"}, {"gloss": "ROBO"}, {"gloss": "AYUDA"}]
...
4. Tras 2,000 milisegundos:
    - Index avanza a 1 (glosa "ROBO"). Buscamos -> "ThumbDown".
    - El widget hace crossfade mágico y comienza "ThumbDown".
...
5. Editar la variable `_animationMap` uniendo en código duro tu glosa con el clip de Blender (`'ROBAR' : 'NombreDeAnimacionDeRoboEnBlender'`).
